...
from books.infrastructure.telethon.row import Row, RowAttribute
...
@dataclass
class Transaction(Sequence[Row]):
    id: UUID
    ...
    def __iter__(self) -> Iterator[Row]:
        return iter(self._rows_in_transaction)

    def __len__(self) -> int:
        return len(self._rows_in_transaction)
    ...
    def commit(self) -> TransactionResult:
        ...
        if conflict is not None:
            self.rollback()
            return TransactionResult(transaction_id=self.id, conflict=conflict)

        self._is_commited = True

        self._link_superset(self._left)
        self._unlink_left()
        self._unlink_subset()

        return TransactionResult(transaction_id=self.id, conflict=None)
    ...
    def _transactions_with_possible_conflict(
        ...
        for transaction in chain(self._left, self._subset):
            if transaction._is_commited:  # noqa: SLF001
                yield transaction

    def _row_ids(self) -> frozenset[RowAttribute]:
        return frozenset(row.id for row in self._rows_in_transaction)
